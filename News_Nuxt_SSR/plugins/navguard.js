...
export default function ({ app }) {
  app.router.beforeEach(function (to, from, next) {
    ...
    var authPath = ["/admin/main", "/admin/newsmanage"];

    //是否需要鉴权操作同行的页面路径数组
    if (authPath.indexOf(to.path) != -1) { //authPath.includes(to.path)
      axios({
        url: app.store.state.domain + "/admin/ucenter/getsess",
        method: "get",
        ...
        .then(function (response) {
          ...
          if (res.errno == 0) {
            ...
            app.store.dispatch("saveadmin", res.data);
            next();
          } else {
            console.log("navguard未登录...");
            app.store.dispatch("logout");
            next({ path: "/admin/adminLogin" });
            // next();
            //   app.router.push({path:"/ucenter/login"});
          }
        })
